...
namespace _03._MOBA_Challenger
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (var player0 in playersStats.OrderByDescending(x => x.Value.Values.Sum()).ThenBy(x => x.Key))
            {
                Console.WriteLine($"{player0.Key}: {player0.Value.Values.Sum()} skill");
                foreach (var pair in player0.Value.OrderByDescending(x => x.Value).ThenBy(x => x.Key))
                {
